...
double time_checkVocab(const std::vector<std::string>& words, const std::string& string,
                            const std::function<void(const std::string&, const std::vector<std::string>&)>& checkVocabulary){

    typedef std::chrono::high_resolution_clock Clock;
    auto t1 = Clock::now();

    checkVocabulary(string, words );
...
void testLevenshteinDistance(const std::string& filename, const std::string& string, const int& iterations){
    ...
    std::cout<< "the string we're searching for is "<< string <<" in "<< filename <<std::endl;
    outfile <<"Selected string: " << string<< " searched in file: " << filename << std::endl;
    ...
    for(int i=0; i< iterations; i++ ) {
        ...
        double time_seq = time_checkVocab(words, string, checkVocabularySeq);
        ...
        double time_par_crit = time_checkVocab(words, string, checkVocabularyParCrit);
        ...
        double time_par_private = time_checkVocab(words, string, checkVocabularyParPrivate);
...
int main(){
    ...
    testLevenshteinDistance("vocabularies/German.txt",
                            "Korrespondenzbankenabteilungen", 1);
